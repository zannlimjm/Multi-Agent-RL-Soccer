...
def local_train(index, opt, global_model, optimizer, save=False):
    ...
    total_reward = 0 
    while True:
        # print("Model's state_dict:", global_model.state_dict())
        if save:
            if curr_episode % opt.save_interval == 0 and curr_episode > 0:
                torch.save(global_model.state_dict(),opt.saved_path_file,_use_new_zipfile_serialization=False)  # save path
        if curr_episode % opt.print_interval == 0 and curr_episode > 0:    
            ...
            print("Episode {}: Total Loss of {}".format(curr_episode, total_loss))
